...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (isLoggedIn) {
      ...
      setProfileComplete(
        JSON.parse(localStorage.getItem("currentUser")).isComplete
      );
    }
  }, [isLoggedIn]);
  ...
  const handleRegistration = async (username, password) => {
    try {
      const response = await axios.get(
        `https://api-petroforecast-ec6416a1a32f.herokuapp.com/users/check/${username}`
      );
      ...
      if (response.status === 200 && response.data.available) {
        ...
        const registrationResponse = await axios.post(
          "https://api-petroforecast-ec6416a1a32f.herokuapp.com/users/register",
          {
            username: username,
            password: password,
          }
  ...
  const handleLogin = async (username, password) => {
    try {
      const response = await axios.post(
        "https://api-petroforecast-ec6416a1a32f.herokuapp.com/users/login",
        { username, password }
      );
      ...
      if (response.data) {
        ...
        let profileComplete = JSON.parse(
  ...
  const handleUpdateProfile = async (updatedUser) => {
    try {
      const response = await axios.put(
        `https://api-petroforecast-ec6416a1a32f.herokuapp.com/users/update/${updatedUser.userId}`,
        updatedUser
      );
